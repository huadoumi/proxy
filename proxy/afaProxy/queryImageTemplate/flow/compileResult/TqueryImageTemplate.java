package afaProxy.queryImageTemplate;
 ...
 *
 * 修改时间：2018-12-03 06:19 <br/>
 *
 * @author  <br/>
 * @version 1.0 <br/>
 *
 */
public class TqueryImageTemplate extends JScript {
    protected INode startStep;
    protected long startTime;
     
    public TqueryImageTemplate(JavaDict __REQ__, JavaDict __RSP__) {
    ...
    public synchronized INode execute() {
        ...
        if (startStep == null) {
            ...
            __REQ__.setItem("__TRADENAME__", "查询镜像模版信息");
            log(LogLevel.INFO, "log start:" + uuid);
            log(LogLevel.INFO, "开始交易  queryImageTemplate:查询镜像模版信息");
            printTrace();
        ...
        if (step == EndNode.SUSPEND_END) {
            ...
            log(LogLevel.INFO, "结束交易 queryImageTemplate");
    ...
    protected class Step2 implements INode {
        ...
        public INode execute() {
            if (startNode == null) {
            	startTime = System.currentTimeMillis();
                log(LogLevel.valueOf(2), "Step2 查询镜像模版信息");
                startNode = new Node1();
            ...
            if (node instanceof EndNode) {
            	gatherStat("Step2", "查询镜像模版信息", ((EndNode) node).getType(), startTime);
            }
        ...
        private class Node5 implements INode {
            ...
            @Override
            public INode execute() {
                log(LogLevel.valueOf(2), "Step2_Node5 获取镜像信息");
                startTime = System.currentTimeMillis();
                try {
                    TCResult result = P_Communication.P_getImagesInfo();
                    if (result == null) {
                        log(LogLevel.ERROR, "技术组件返回值不能为空");
                    	gatherStat("Step2_Node5", "获取镜像信息", startTime, "技术组件返回值不能为空");
                   ...
                   log(LogLevel.valueOf(2), "逻辑返回值=" + status);
                    ...
                    if (outputParams != null) {
                        if (outputParams.size() != 1) {
                        	...
                        	gatherStat("Step2_Node5", "获取镜像信息", startTime, "出参的实参个数与形参个数不一致");
                            return getExceptionHandler(new Node4());
                        }
                        __RSP__.setItem("jsonStr", outputParams.get(0));
                        logVar(LogLevel.valueOf(4), "出参0", outputParams.get(0));
                    }
                	gatherStat("Step2_Node5", "获取镜像信息", status, startTime);
                    switch (status) {
                    case 1:
                        return new Node3();
                    default:
                	...
                	gatherStat("Step2_Node5", "获取镜像信息", startTime, ExceptionUtils.toDetailString(e));
