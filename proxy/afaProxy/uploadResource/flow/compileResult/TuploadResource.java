package afaProxy.uploadResource;
 ...
 *
 * 修改时间：2019-03-28 02:19 <br/>
 *
 * @author  <br/>
 * @version 1.0 <br/>
 *
 */
public class TuploadResource extends JScript {
    protected INode startStep;
    protected long startTime;
     
    public TuploadResource(JavaDict __REQ__, JavaDict __RSP__) {
    ...
    public synchronized INode execute() {
        ...
        if (startStep == null) {
            ...
            __REQ__.setItem("__TRADENAME__", "上传资源");
            log(LogLevel.INFO, "log start:" + uuid);
            log(LogLevel.INFO, "开始交易  uploadResource:上传资源");
            printTrace();
        ...
        if (step == EndNode.SUSPEND_END) {
            ...
            log(LogLevel.INFO, "结束交易 uploadResource");
    ...
    protected class Step2 implements INode {
        ...
        public INode execute() {
            if (startNode == null) {
            	startTime = System.currentTimeMillis();
                log(LogLevel.valueOf(2), "Step2 部署资源");
                startNode = new Node1();
                setExceptionHandler(null);
            }
            ...
            if (node instanceof EndNode) {
            	gatherStat("Step2", "部署资源", ((EndNode) node).getType(), startTime);
            }
        ...
        private class Node5 implements INode {
            ...
            @Override
            public INode execute() {
                log(LogLevel.valueOf(2), "Step2_Node5 上传资源");
                startTime = System.currentTimeMillis();
                try {
                    JavaList _arg0_ = ((JavaDict)__REQ__.getItem("jsonDict")).getItem("pods");
                    logVar(LogLevel.valueOf(4), "入参0", _arg0_);
                    String _arg1_ = ((JavaDict)__REQ__.getItem("jsonDict")).getItem("upPath")+"/"+((JavaDict)((JavaDict)__REQ__.getItem("jsonDict")).getItem("resource")).getItem("name");
                    logVar(LogLevel.valueOf(4), "入参1", _arg1_);
                    String _arg2_ = ((JavaDict)__REQ__.getItem("jsonDict")).getItem("restart");
                    logVar(LogLevel.valueOf(4), "入参2", _arg2_);
                    String _arg3_ = ((JavaDict)__REQ__.getItem("jsonDict")).getItem("autostart");
                    logVar(LogLevel.valueOf(4), "入参3", _arg3_);
                    String _arg4_ = ((JavaDict)__REQ__.getItem("jsonDict")).getItem("groupName");
                    logVar(LogLevel.valueOf(4), "入参4", _arg4_);
                    TCResult result = P_Communication.P_uploadResource(_arg0_, _arg1_, _arg2_, _arg3_, _arg4_);
                    if (result == null) {
                        log(LogLevel.ERROR, "技术组件返回值不能为空");
                    	gatherStat("Step2_Node5", "上传资源", startTime, "技术组件返回值不能为空");
                        return getExceptionHandler(new Node4());
                    }
                   ...
                   log(LogLevel.valueOf(2), "逻辑返回值=" + status);
                    ...
                    if (outputParams != null) {
                        if (outputParams.size() != 1) {
                            log(LogLevel.ERROR, "出参的实参个数与形参个数不一致");
                        	gatherStat("Step2_Node5", "上传资源", startTime, "出参的实参个数与形参个数不一致");
                            return getExceptionHandler(new Node4());
                        }
                        __RSP__.setItem("rspDict", outputParams.get(0));
                        logVar(LogLevel.valueOf(4), "出参0", outputParams.get(0));
                    }
                	gatherStat("Step2_Node5", "上传资源", status, startTime);
                    switch (status) {
                    case 1:
                        return new Node6();
                    default:
                	...
                	gatherStat("Step2_Node5", "上传资源", startTime, ExceptionUtils.toDetailString(e));
                    setGlobalError("E", "ACMP0E001", e.toString());
                    log(LogLevel.ERROR, e);
                    INode exceptionHandler = getExceptionHandler(new Node4());
                    if (exceptionHandler == null) {
        ...
        private class Node6 implements INode {
            ...
            @Override
            public INode execute() {
                log(LogLevel.valueOf(2), "Step2_Node6 清理上传目录");
                startTime = System.currentTimeMillis();
                try {
                    ...
                    if (result == null) {
                        log(LogLevel.ERROR, "技术组件返回值不能为空");
                    	gatherStat("Step2_Node6", "清理上传目录", startTime, "技术组件返回值不能为空");
                        return getExceptionHandler(new Node4());
                    }
                   ...
                   log(LogLevel.valueOf(2), "逻辑返回值=" + status);
                	...
                	gatherStat("Step2_Node6", "清理上传目录", status, startTime);
                    switch (status) {
                    case 1:
                        return new Node3();
                    default:
                	...
                	gatherStat("Step2_Node6", "清理上传目录", startTime, ExceptionUtils.toDetailString(e));
                    setGlobalError("E", "ACMP0E001", e.toString());
                    log(LogLevel.ERROR, e);
                    INode exceptionHandler = getExceptionHandler(new Node4());
                    if (exceptionHandler == null) {
    ...
    protected class Step3 implements INode {
        ...
        private class Node5 implements INode {
            ...
            @Override
            public INode execute() {
                log(LogLevel.valueOf(2), "Step3_Node5 javaDict转换json串");
                startTime = System.currentTimeMillis();
                try {
                    ...
                    if (result == null) {
                        log(LogLevel.ERROR, "技术组件返回值不能为空");
                    	gatherStat("Step3_Node5", "javaDict转换json串", startTime, "技术组件返回值不能为空");
                        return getExceptionHandler(new Node4());
                    }
                   ...
                   log(LogLevel.valueOf(2), "逻辑返回值=" + status);
                    ...
                    if (outputParams != null) {
                        if (outputParams.size() != 1) {
                            log(LogLevel.ERROR, "出参的实参个数与形参个数不一致");
                        	gatherStat("Step3_Node5", "javaDict转换json串", startTime, "出参的实参个数与形参个数不一致");
                            return getExceptionHandler(new Node4());
                        }
                        __RSP__.setItem("jsonStr", outputParams.get(0));
                        logVar(LogLevel.valueOf(4), "出参0", outputParams.get(0));
                    }
                	gatherStat("Step3_Node5", "javaDict转换json串", status, startTime);
                    switch (status) {
                    case 1:
                        return new Node6();
                    default:
                	...
                	gatherStat("Step3_Node5", "javaDict转换json串", startTime, ExceptionUtils.toDetailString(e));
                    setGlobalError("E", "ACMP0E001", e.toString());
                    log(LogLevel.ERROR, e);
                    INode exceptionHandler = getExceptionHandler(new Node4());
                    if (exceptionHandler == null) {
        ...
        private class Node6 implements INode {
            ...
            @Override
            public INode execute() {
                log(LogLevel.valueOf(2), "Step3_Node6 HTTP响应报文拼包");
                startTime = System.currentTimeMillis();
                try {
                    String _arg0_ = "200";
                    logVar(LogLevel.valueOf(4), "入参0", _arg0_);
                    ...
                    if (result == null) {
                        log(LogLevel.ERROR, "技术组件返回值不能为空");
                    	gatherStat("Step3_Node6", "HTTP响应报文拼包", startTime, "技术组件返回值不能为空");
                        return getExceptionHandler(new Node4());
                    }
                   ...
                   log(LogLevel.valueOf(2), "逻辑返回值=" + status);
                    ...
                    if (outputParams != null) {
                        if (outputParams.size() != 1) {
                            log(LogLevel.ERROR, "出参的实参个数与形参个数不一致");
                        	gatherStat("Step3_Node6", "HTTP响应报文拼包", startTime, "出参的实参个数与形参个数不一致");
                            return getExceptionHandler(new Node4());
                        }
                        __RSP__.setItem("__SNDPCK__", outputParams.get(0));
                        logVar(LogLevel.valueOf(4), "出参0", outputParams.get(0));
                    }
                	gatherStat("Step3_Node6", "HTTP响应报文拼包", status, startTime);
                    switch (status) {
                    case 1:
                        return new Node3();
                    default:
                	...
                	gatherStat("Step3_Node6", "HTTP响应报文拼包", startTime, ExceptionUtils.toDetailString(e));
                    setGlobalError("E", "ACMP0E001", e.toString());
                    log(LogLevel.ERROR, e);
                    INode exceptionHandler = getExceptionHandler(new Node4());
                    if (exceptionHandler == null) {
