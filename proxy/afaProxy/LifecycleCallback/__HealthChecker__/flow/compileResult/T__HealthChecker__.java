package afaProxy.__HealthChecker__;
 ...
 * 交易描述：服务健康检测场景 <br/>
 *
 * 创建时间：2019-06-17 08:14 <br/>
 *
 * 修改时间：2019-06-17 08:14 <br/>
 *
 * @author  <br/>
 * @version 1.0 <br/>
 *
 */
public class T__HealthChecker__ extends JScript {
    protected INode startStep;
    protected long startTime;
     
    public T__HealthChecker__(JavaDict __REQ__, JavaDict __RSP__) {
    ...
    public synchronized INode execute() {
        ...
        if (startStep == null) {
            ...
            __REQ__.setItem("__TRADENAME__", "服务健康检测场景");
            log(LogLevel.INFO, "log start:" + uuid);
            log(LogLevel.INFO, "开始交易  __HealthChecker__:服务健康检测场景");
            printTrace();
        ...
        if (step == EndNode.SUSPEND_END) {
            ...
            log(LogLevel.INFO, "结束交易 __HealthChecker__");
    ...
    protected class Step1 implements INode {
        ...
        public INode execute() {
            if (startNode == null) {
            	startTime = System.currentTimeMillis();
                log(LogLevel.valueOf(2), "Step1 check");
                startNode = new Node1();
            ...
            gatherStat("Step1", "check", 1, startTime);
