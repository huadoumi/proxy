package afaProxy.queryImageTagList;
 ...
 * 交易描述：查询镜像的历史版本 <br/>
 *
 * 创建时间：2018-11-30 11:08 <br/>
 *
 * 修改时间：2018-12-20 03:41 <br/>
 *
 * @author  <br/>
 * @version 1.0 <br/>
 *
 */
public class TqueryImageTagList extends JScript {
    protected INode startStep;
    protected long startTime;
     
    public TqueryImageTagList(JavaDict __REQ__, JavaDict __RSP__) {
    ...
    public synchronized INode execute() {
        ...
        if (startStep == null) {
            ...
            __REQ__.setItem("__TRADENAME__", "查询镜像的历史版本");
            log(LogLevel.INFO, "log start:" + uuid);
            log(LogLevel.INFO, "开始交易  queryImageTagList:查询镜像的历史版本");
            printTrace();
        ...
        if (step == EndNode.SUSPEND_END) {
            ...
            log(LogLevel.INFO, "结束交易 queryImageTagList");
    ...
    protected class Step1 implements INode {
        ...
        private class Node1 implements INode {
            ...
            @Override
            public INode execute() {
                log(LogLevel.valueOf(2), "Step1_Node1 默认逻辑错误委托");
        ...
        private class Node2 implements INode {
            ...
            @Override
            public INode execute() {
                log(LogLevel.valueOf(2), "Step1_Node2 正常结束");
        ...
        private class Node3 implements INode {
            ...
            @Override
            public INode execute() {
                log(LogLevel.valueOf(2), "Step1_Node3 开始");
                return new Node1();
            }    
        }
                ...
                try {
                   ...
                   log(LogLevel.valueOf(2), "逻辑返回值=" + status);
                    ...
                    if (outputParams != null) {
                        ...
                        __REQ__.setItem("proxyJson", outputParams.get(0));
        ...
        private class Node6 implements INode {
            ...
            @Override
            public INode execute() {
                log(LogLevel.valueOf(2), "Step1_Node6 json串转换javaDict");
                startTime = System.currentTimeMillis();
                try {
                    JavaDict _arg0_ = __REQ__.getItem("proxyJson");
                    logVar(LogLevel.valueOf(4), "入参0", _arg0_);
                    String _arg1_ = __REQ__.getItem("proxyJsonStr");
                    logVar(LogLevel.valueOf(4), "入参1", _arg1_);
                    TCResult result = P_Json.strToDict(_arg0_, _arg1_);
                    if (result == null) {
                        log(LogLevel.ERROR, "技术组件返回值不能为空");
                    	gatherStat("Step1_Node6", "json串转换javaDict", startTime, "技术组件返回值不能为空");
                        return getExceptionHandler(new Node4());
                    }
                   ...
                   log(LogLevel.valueOf(2), "逻辑返回值=" + status);
                	...
                	gatherStat("Step1_Node6", "json串转换javaDict", status, startTime);
                    switch (status) {
                    case 1:
                        return new Node2();
                    default:
                	...
                	gatherStat("Step1_Node6", "json串转换javaDict", startTime, ExceptionUtils.toDetailString(e));
                    setGlobalError("E", "ACMP0E001", e.toString());
                    log(LogLevel.ERROR, e);
                    INode exceptionHandler = getExceptionHandler(new Node4());
                    if (exceptionHandler == null) {
        ...
        private class Node7 implements INode {
            ...
            @Override
            public INode execute() {
                log(LogLevel.valueOf(2), "Step1_Node7 提取请求报文体");
                startTime = System.currentTimeMillis();
                try {
                    ...
                    if (result == null) {
                        log(LogLevel.ERROR, "技术组件返回值不能为空");
                    	gatherStat("Step1_Node7", "提取请求报文体", startTime, "技术组件返回值不能为空");
                        return getExceptionHandler(new Node4());
                    }
                   ...
                   log(LogLevel.valueOf(2), "逻辑返回值=" + status);
                    ...
                    if (outputParams != null) {
                        if (outputParams.size() != 1) {
                            log(LogLevel.ERROR, "出参的实参个数与形参个数不一致");
                        	gatherStat("Step1_Node7", "提取请求报文体", startTime, "出参的实参个数与形参个数不一致");
                            return getExceptionHandler(new Node4());
                        }
                        __REQ__.setItem("proxyJsonStr", outputParams.get(0));
                        logVar(LogLevel.valueOf(4), "出参0", outputParams.get(0));
                    }
                	gatherStat("Step1_Node7", "提取请求报文体", status, startTime);
                    switch (status) {
                    case 1:
                        return new Node6();
                    default:
                	...
                	gatherStat("Step1_Node7", "提取请求报文体", startTime, ExceptionUtils.toDetailString(e));
                    setGlobalError("E", "ACMP0E001", e.toString());
                    log(LogLevel.ERROR, e);
                    INode exceptionHandler = getExceptionHandler(new Node4());
                    if (exceptionHandler == null) {
    ...
    protected class Step2 implements INode {
        ...
        private class Node3 implements INode {
            ...
            @Override
            public INode execute() {
                log(LogLevel.valueOf(2), "Step2_Node3 获取镜像的tag列表");
                startTime = System.currentTimeMillis();
                try {
                    String _arg0_ = ((JavaDict)__REQ__.getItem("proxyJson")).getItem("imageName");
                    logVar(LogLevel.valueOf(4), "入参0", _arg0_);
                    TCResult result = P_Communication.P_getTagList(_arg0_);
                    if (result == null) {
                        log(LogLevel.ERROR, "技术组件返回值不能为空");
                    	gatherStat("Step2_Node3", "获取镜像的tag列表", startTime, "技术组件返回值不能为空");
                        return getExceptionHandler(new Node4());
                    }
                   ...
                   log(LogLevel.valueOf(2), "逻辑返回值=" + status);
                    ...
                    if (outputParams != null) {
                        if (outputParams.size() != 1) {
                            log(LogLevel.ERROR, "出参的实参个数与形参个数不一致");
                        	gatherStat("Step2_Node3", "获取镜像的tag列表", startTime, "出参的实参个数与形参个数不一致");
                            return getExceptionHandler(new Node4());
                        }
                        __RSP__.setItem("resp", outputParams.get(0));
                        logVar(LogLevel.valueOf(4), "出参0", outputParams.get(0));
                    }
                	gatherStat("Step2_Node3", "获取镜像的tag列表", status, startTime);
                    switch (status) {
                    case 1:
                        return new Node5();
                    default:
                	...
                	gatherStat("Step2_Node3", "获取镜像的tag列表", startTime, ExceptionUtils.toDetailString(e));
                    setGlobalError("E", "ACMP0E001", e.toString());
                    log(LogLevel.ERROR, e);
                    INode exceptionHandler = getExceptionHandler(new Node4());
                    if (exceptionHandler == null) {
